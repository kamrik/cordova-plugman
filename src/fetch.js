...
module.exports = function fetchPlugin(plugin_src, plugins_dir, options) {
    ...
    options.search_path = options.search_path || [];
    ...
    if ( uri.protocol && uri.protocol != 'file:' && !plugin_src.match(/^\w+:\\/)) {
        ...
        // If it's not a network URL, it's either a local path or plugin id.
        ...
        if (fs.existsSync(plugin_dir)) {
            ...
            var local_dir = findLocalPlugin(plugin_src, options.search_path);
            if(local_dir) {
                p = Q(local_dir);
            } else {
...
function findLocalPlugin(plugin_id, search_path) {
    for(var i = 0; i < search_path.length; i++){
        var files = fs.readdirSync(search_path[i]);
        for (var j = 0; j < files.length; j++){
            var plugin_path = path.join(search_path[i], files[j]);
            try {
